-- AI CAPTCHA project schema v1.4
-- MySQL 8.0 / InnoDB / utf8mb4
--
-- CAPTCHA rules:
-- - type1_drag: drag CAPTCHA using ASCII art raster image options.
-- - type2_identify: identify fallback CAPTCHA using an ASCII art raster question image and text choices.
-- - The AI model is a drag trajectory bot/human classifier.
-- - CAPTCHA images sent to clients are raster images, not raw ASCII text.
-- - Server-side answer keys and audit snapshots must not be exposed to clients.
...
CREATE TABLE IF NOT EXISTS captcha_images (
  ...
  filename VARCHAR(2048) NOT NULL COMMENT 'Raster image path shown to the client.',
  label VARCHAR(128) NOT NULL,
  instruction VARCHAR(255) NULL COMMENT 'Mainly used for question images.',
  ascii_txt_path VARCHAR(2048) NULL COMMENT 'Internal QA artifact only; do not expose raw ASCII text to clients.',
...
CREATE TABLE IF NOT EXISTS captchas (
  ...
  answer_payload JSON NULL COMMENT 'Server-side only. Do not include in client responses.',
...
CREATE TABLE IF NOT EXISTS captcha_options (
  ...
  is_correct_server_side BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'Server-side only. Never include in client responses.',
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS captcha_verifications (
  ...
  server_answer_snapshot JSON NULL COMMENT 'Server-side audit only. Do not expose to clients.',
